...
import { Card, Button, InputGroup, Input, Label, Spinner } from "@heroui/react";
...
function SigninContent() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Loading state remains true during OAuth redirect
  };
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="z-10 w-full max-w-md sm:max-w-lg lg:max-w-xl">
        <Card className="w-full p-8 sm:p-10 shadow-[0_20px_60px_rgba(0,0,0,0.8)] border border-white/5 bg-[#0a0a0c]/80 backdrop-blur-2xl rounded-[32px]">
          ...
          <div className="flex flex-col items-center justify-center gap-2 pb-8 border-b border-white/5 mb-8 text-center relative">
            ...
            <h1 className="text-2xl sm:text-3xl font-display font-bold tracking-tight text-white">Welcome back</h1>
            <p className="text-sm text-zinc-400">Sign in to your NeuPrompt account.</p>
          </div>
          ...
          <form onSubmit={handleSignin} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-3 border border-white/10 rounded-2xl px-4 py-1 bg-[#030303] focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/50 transition-all shadow-inner">
                ...
                <input required type={isVisible ? "text" : "password"} placeholder="Your password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-transparent py-2.5 text-sm outline-none border-none text-white placeholder:text-zinc-600" />
            ...
            {success && <div className="px-4 py-3 text-sm font-medium rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{success}</div>}

            <Button type="submit" isLoading={isLoading} className="w-full font-semibold rounded-2xl text-sm h-12 bg-blue-600 text-white shadow-[0_0_20px_rgba(37,99,235,0.2)] hover:bg-blue-500 transition-all border border-blue-500/50 mt-2">
              Sign In
            </Button>
          </form>
          ...
          <div className="flex items-center gap-3 my-8">
            ...
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono">or continue with</span>
            <div className="flex-1 h-px bg-white/5" />
          </div>
          ...
          <div className="text-center pt-8 border-t border-white/5 mt-8 text-sm text-zinc-500">
            ...
            <Link href={`/auth/signup?redirect=${redirectTo}`} className="font-medium text-blue-400 hover:text-blue-300 transition-colors">
              Sign up
            </Link>
          </div>
